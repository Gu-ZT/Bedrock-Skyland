...
public class TargetBlock {
    ...
    public TargetBlock(@NotNull BlockPos pos, ClientLevel world) {
        this.hasTried = false;
        ...
        if (redstoneTorchBlockPos == null) {
            ...
            if (slimeBlockPos != null) {
                BlockPlacer.simpleBlockPlacement(slimeBlockPos, Blocks.PISTON);
    ...
    private void updateStatus() {
        ...
        }  else if (this.world.getBlockState(this.pistonBlockPos).is(Blocks.PISTON) && !this.world.getBlockState(this.pistonBlockPos).getValue(PistonBaseBlock.EXTENDED) && !CheckingEnvironment.findNearbyRedstoneTorch(this.world, this.pistonBlockPos).isEmpty() && this.world.getBlockState(this.blockPos).is(Blocks.BEDROCK)) {
        ...
        } else if (this.world.getBlockState(this.pistonBlockPos).is(Blocks.PISTON) && this.world.getBlockState(this.pistonBlockPos).getValue(PistonBaseBlock.FACING) == Direction.DOWN && !this.world.getBlockState(this.pistonBlockPos).getValue(PistonBaseBlock.EXTENDED) && !CheckingEnvironment.findNearbyRedstoneTorch(this.world, this.pistonBlockPos).isEmpty() && this.world.getBlockState(this.blockPos).is(Blocks.BEDROCK)) {
            this.status = Status.STUCK;
            this.hasTried = false;
            this.stuckTicksCounter = 0;
        }else if (this.world.getBlockState(this.pistonBlockPos).is(Blocks.PISTON) && !this.world.getBlockState(this.pistonBlockPos).getValue(PistonBaseBlock.EXTENDED) && this.world.getBlockState(this.pistonBlockPos).getValue(PistonBaseBlock.FACING) == Direction.UP && CheckingEnvironment.findNearbyRedstoneTorch(this.world, this.pistonBlockPos).isEmpty() && this.world.getBlockState(this.blockPos).is(Blocks.BEDROCK)) {
